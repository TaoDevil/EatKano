...
function gameTime() {
    _gameTimeNum--;
    if (_gameTimeNum <= 0) {
        GameTimeLayer.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;松哥制裁时间到！';
        gameOver();
...
function showGameScoreLayer() {
    ...
    document.getElementById('GameScoreLayer-bast').innerHTML = '最作丑&nbsp;&nbsp;' + bast;
    l.style.display = 'block';
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return '这都不会做？';
    if (score <= 99) return '拜在这现世！';
    if (score <= 149) return '是不是作打？';
    if (score <= 199) return '你肾莫意思？';
    return '作！';
}
